...
class Alg_WC_MSRP_Core {
	...
	function __construct() {
		if ( 'yes' === get_option( 'alg_wc_msrp_plugin_enabled', 'yes' ) ) {
			...
			if ( is_admin() ) {
				...
				// Quick and bulk edit
				if (
					$this->options['do_add_admin_quick_edit'] ||
					$this->options['do_add_admin_bulk_edit']
				) {
					if ( $this->options['do_add_admin_quick_edit'] ) {
						add_action(
							'woocommerce_product_quick_edit_' . $this->options['admin_quick_bulk_edit_position'],
					...
					if ( $this->options['do_add_admin_bulk_edit'] ) {
						add_action(
							'woocommerce_product_bulk_edit_'  . $this->options['admin_quick_bulk_edit_position'],
							array( $this, 'add_bulk_and_quick_edit_fields' ),
							PHP_INT_MAX
						);
					}
				...
				// Cart total savings
				if (
					$this->options['cart_total_savings_enabled'] &&
					! empty( $this->options['cart_total_savings_positions'] )
				) {
	...
	function init_options() {
		...
		foreach ( $sections as $section_id ) {
			...
			$text_format = get_option( $option_id . '_text_format', 'del' );
			if ( '' === $text_format ) {
				$template = '<div class="price"><label for="alg_wc_msrp">MSRP</label>: <span id="alg_wc_msrp">%msrp%%you_save%</span></div>';
			}else{
				$template = '<div class="price"><label for="alg_wc_msrp">MSRP</label>: <span id="alg_wc_msrp"><' . $text_format . '>%msrp%</' . $text_format . '>%you_save%</span></div>';
			}
	 ...
	 * @since   1.3.6
	 *
	 * @todo    (fix) "Sale" tag is still displayed: maybe use `woocommerce_product_is_on_sale` filter to hide it (optionally)
	 * @todo    (dev) add custom template (i.e., `'%sale_price%'`) (#13210)
	 */
	function hide_regular_price_for_sale_products( $price, $product ) {
		if (
			$product->is_on_sale() &&
			$this->is_msrp_set_for_product( $product )
		) {
			if ( $product->is_type( 'variable' ) ) {
				// Variable products
		...
		if ( isset( WC()->cart ) ) {
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( ! empty( $msrp_data['msrp'] ) ) {
					$savings += (
						( $cart_item['data']->get_price() - $msrp_data['msrp'] ) *
						$cart_item['quantity']
					);
				}
			}
			if ( ! empty( $savings ) ) {
				echo wp_kses_post(
					str_replace(
						'%total_savings%',
						wc_price( -$savings ),
						do_shortcode( $this->options['cart_total_savings_template'] )
					)
				);
			}
		}
	 ...
	 * @todo    (dev) reposition this (to the `price_fields` section)?
	 * @todo    (dev) actual value (instead of "No change" placeholder) (probably need to add value to `woocommerce_inline_`)?
	 * @todo    (feature) add `_alg_msrp_by_country` and `_alg_msrp_by_currency`
	 */
	function add_bulk_and_quick_edit_fields() {
		$current_filter = current_filter();
		if (
			'end' === $this->options['admin_quick_bulk_edit_position'] &&
			'woocommerce_product_quick_edit_end' === $current_filter
		) {
			echo '<br class="clear" />';
		}
		?><label>
			<span class="title"><?php esc_html_e( 'MSRP', 'msrp-for-woocommerce' ); ?></span>
			<span class="input-text-wrap">
				<input type="text" name="_alg_msrp" class="text wc_input_price" placeholder="<?php esc_attr_e( '- No change -', 'msrp-for-woocommerce' ); ?>" value="">
			</span>
		</label><?php
		if (
			'start' === $this->options['admin_quick_bulk_edit_position'] &&
			'woocommerce_product_quick_edit_start' === $current_filter
		) {
			echo '<br class="clear" />';
		}
	}
	...
	function save_bulk_and_quick_edit_fields( $post_id, $post ) {
		...
		if (
			...
			! wp_verify_nonce(
				sanitize_text_field( wp_unslash( $_REQUEST['woocommerce_quick_edit_nonce'] ) ),
				'woocommerce_quick_edit_nonce'
			)
		) {
			return $post_id;
		}
		...
		// Save
		if (
			isset( $_REQUEST['_alg_msrp'] ) &&
			'' !== $_REQUEST['_alg_msrp']
		) {
			update_post_meta(
				$post_id,
				'_alg_msrp',
				sanitize_text_field( wp_unslash( $_REQUEST['_alg_msrp'] ) )
			);
		}
	...
	function render_product_columns( $column ) {
		if ( 'msrp' === $column ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				echo wp_kses_post( $this->get_variable_msrp( $product ) );
			} else {
				...
				if ( '' != $msrp_data['msrp'] ) {
					echo wp_kses_post(
						wc_price(
							$msrp_data['msrp'],
							array( 'currency' => $msrp_data['currency'] )
						)
					);
				}
			}
	...
	function add_msrp_input_variation( $loop, $variation_data, $variation ) {
		...
		woocommerce_wp_text_input( array(
			...
			'value'         => wc_format_localized_price( $variation_data['_alg_msrp'][0] ?? '' ),
			'label'         => $this->get_admin_field_label(),
			'data_type'     => 'price',
		...
		if ( ! empty( $this->options['msrp_countries'] ) ) {
			$values = (
				(
					isset( $variation_data['_alg_msrp_by_country'][0] ) &&
					is_serialized( $variation_data['_alg_msrp_by_country'][0] )
				) ?
				unserialize( $variation_data['_alg_msrp_by_country'][0] ) :
				array()
			);
			foreach ( $this->options['msrp_countries'] as $country_code ) {
				$currency = ( $this->options['msrp_countries_currencies'][ $country_code ] ?? '' );
				...
				woocommerce_wp_text_input( array(
					...
					'label'         => $this->get_admin_field_label( $currency, $country_code ),
					'data_type'     => 'price',
					'wrapper_class' => 'form-row form-row-full',
				) );
			}
		...
		if ( ! empty( $this->options['msrp_currencies'] ) ) {
			$values = (
				(
					isset( $variation_data['_alg_msrp_by_currency'][0] ) &&
					is_serialized( $variation_data['_alg_msrp_by_currency'][0] )
				) ?
				unserialize( $variation_data['_alg_msrp_by_currency'][0] ) :
				array()
			);
			foreach ( $this->options['msrp_currencies'] as $currency ) {
				...
				woocommerce_wp_text_input( array(
					...
					'value'         => wc_format_localized_price( $value ),
					'label'         => $this->get_admin_field_label( $currency ),
					'data_type'     => 'price',
					'wrapper_class' => 'form-row form-row-full',
				) );
			}
		...
		// Nonce
		wp_nonce_field(
			"alg_wc_msrp_variation_{$loop}_nonce",
			"_alg_wc_msrp_variation_{$loop}_nonce"
		);
	...
	function save_msrp_input_variation( $variation_id, $i ) {
		if ( isset( $_POST['variable_alg_msrp'][ $i ] ) ) {
			...
			// Check nonce
			if (
				! isset( $_REQUEST[ "_alg_wc_msrp_variation_{$i}_nonce" ] ) ||
				! wp_verify_nonce(
					sanitize_text_field( wp_unslash( $_REQUEST[ "_alg_wc_msrp_variation_{$i}_nonce" ] ) ),
			...
			update_post_meta(
				$variation_id,
				'_alg_msrp',
				sanitize_text_field( wp_unslash( $_POST['variable_alg_msrp'][ $i ] ) )
			);

			// Update MSRP by country
			if ( isset( $_POST['variable_alg_msrp_by_country'][ $i ] ) ) {
				update_post_meta(
					$variation_id,
					'_alg_msrp_by_country',
					wc_clean( wp_unslash( $_POST['variable_alg_msrp_by_country'][ $i ] ) ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				);
			}

			// Update MSRP by currency
			if ( isset( $_POST['variable_alg_msrp_by_currency'][ $i ] ) ) {
				update_post_meta(
					$variation_id,
					'_alg_msrp_by_currency',
					wc_clean( wp_unslash( $_POST['variable_alg_msrp_by_currency'][ $i ] ) ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				);
			}
	...
	function add_msrp_input() {
		...
		// Nonce
		wp_nonce_field( 'alg_wc_msrp_nonce', '_alg_wc_msrp_nonce' );
	...
	function save_msrp_input( $post_id, $__post ) {
		if (
			isset( $_POST['_alg_msrp'] ) &&
			empty( $_REQUEST['woocommerce_quick_edit'] )
		) {

			// Check nonce
			if (
				! isset( $_REQUEST['_alg_wc_msrp_nonce'] ) ||
				! wp_verify_nonce(
					sanitize_text_field( wp_unslash( $_REQUEST['_alg_wc_msrp_nonce'] ) ),
					'alg_wc_msrp_nonce'
				)
			) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			...
			update_post_meta(
				$post_id,
				'_alg_msrp',
				sanitize_text_field( wp_unslash( $_POST['_alg_msrp'] ) )
			);
			...
			if ( isset( $_POST['_alg_msrp_by_country'] ) ) {
				update_post_meta(
					$post_id,
					'_alg_msrp_by_country',
					wc_clean( wp_unslash( $_POST['_alg_msrp_by_country'] ) ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				);
			}
			...
			if ( isset( $_POST['_alg_msrp_by_currency'] ) ) {
				update_post_meta(
					$post_id,
					'_alg_msrp_by_currency',
					wc_clean( wp_unslash( $_POST['_alg_msrp_by_currency'] ) ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				);
			}
	...
	function get_product_id( $_product ) {
		...
		return (
			$this->options['is_wc_version_below_3_0_0'] ?
			( $_product->variation_id ?? $_product->id ) :
			$_product->get_id()
		);
	}
	...
	function get_visitors_country_by_ip() {
		...
		$this->visitors_country_by_ip = ( $location['country'] ?? '' );
		return $this->visitors_country_by_ip;
	}
	 ...
	 * @since   1.1.0
	 *
	 * @todo    (dev) automatic currency conversion (i.e., exchange rates)
	 */
	function get_msrp( $product_id ) {
		...
		if (
			( '' == $msrp || 0 == $msrp ) &&
			! empty( $this->options['msrp_currencies'] )
		) {
		...
		return apply_filters(
			'alg_wc_get_msrp',
			array( 'msrp' => $msrp, 'currency' => $currency ),
			$product_id
		);
	}
	 ...
	 *
	 * @todo    (dev) optionally return in `$child_id => $child_object` format?
	 */
	...
	function get_saved_value_variable( $product, $type, $you_save_round ) {
		switch ( $this->options['variable_optimization'] ) {
			case 'in_transients':
				...
				return ( $transient_value[ $transient_hash ] ?? false );
			case 'in_array':
				$product_id = $this->get_product_id( $product );
				return ( $this->saved_get_variable_msrp[ $product_id ][ $type ][ $you_save_round ] ?? false );
		}
		return false; // `none`
	}
	 ...
	 * @since   1.0.0
	 *
	 * @todo    (fix) add `%product_id%` placeholder to the template (and replace `id="alg_wc_msrp"` with `id="alg_wc_msrp-%product_id%" class="alg_wc_msrp"`)
	 * @todo    (dev) calculate placeholders only if search value is present in the template (e.g., search for `%price%` before calling `get_product_price_html()`)
	 * @todo    (dev) add `%raw_msrp%` placeholder?
	 * @todo    (dev) add `%regular_price%` and `%sale_price%` placeholders?
	 */
	function display( $price_html, $product, $section_id = false ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			if (
				( 'show_if_diff'   == $display && $msrp == $price ) ||
				( 'show_if_higher' == $display && $msrp <= $price )
			) {
				return $price_html;
			}
			$diff = $msrp - $price;
		...
		$msrp_html = str_replace(
			array_keys( $replaced_values ),
			$replaced_values,
			do_shortcode( $this->options['msrp_display'][ $section_id ]['template'] )
		);

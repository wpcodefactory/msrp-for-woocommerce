...
class Alg_WC_MSRP_Settings_Admin_Advanced extends Alg_WC_MSRP_Settings_Section {
	...
	/**
	 * Tracks the number of modified products.
	 *
	 * @version 1.8.0
	 * @since   1.8.0
	 */
	 ...
	 * @since   1.3.9
	 *
	 * @todo    (dev) add "Tools" to the title?
	 */
	function __construct() {
		...
		add_action(
			'admin_init',
			array( $this, 'admin_actions' ),
			PHP_INT_MAX
		);

		add_action(
			'woocommerce_admin_field_' . 'alg_wc_msrp_tools',
			array( $this, 'output_tools' ),
			PHP_INT_MAX
		);
		...
		<tr valign="top">
			...
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
				<?php
					...
					$allowed_html['a']['onclick'] = array();
					echo wp_kses( $value['desc'], $allowed_html );
				?>
			</td>
		</tr>
		<?php
	 ...
	 * @since   1.3.4
	 *
	 * @todo    (dev) better (i.e., more informative) notices?
	 */
	function admin_actions_notice() {
		echo '<div class="notice notice-success is-dismissible"><p>' .
			sprintf(
				/* Translators: %d: Number of products. */
				esc_html__( '%d product(s) modified.', 'msrp-for-woocommerce' ),
				(int) $this->alg_wc_msrp_action_done
			) .
		'</p></div>';
	}
	...
	function admin_actions() {
		if ( ! empty( $_GET['alg_wc_msrp_action'] ) ) {
			...
			// Check nonce
			if (
				! isset( $_GET['alg_wc_msrp_wpnonce'] ) ||
				! wp_verify_nonce(
					sanitize_text_field( wp_unslash( $_GET['alg_wc_msrp_wpnonce'] ) ),
			...
			switch ( $_GET['alg_wc_msrp_action'] ) {
				...
				// Copy all products prices to MSRP
				...
				// Delete all products MSRP meta
				case 'delete_msrp_meta':
					global $wpdb;
					$plugin_meta = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
						"SELECT * FROM {$wpdb->postmeta} WHERE meta_key LIKE '_alg_msrp%'"
					);
	...
	function get_user_roles() {
		...
		$user_roles = array_merge(
			array(
				'guest' => array(
					'name'         => __( 'Guest', 'msrp-for-woocommerce' ),
					'capabilities' => array(),
				),
			),
			apply_filters(
	...
	function get_tools() {
		...
		foreach ( $tools_data as $tool_action => $tool_title ) {
			$tools[] = '<a' .
				' class="button"' .
				' href="' . add_query_arg(
					array(
						'alg_wc_msrp_action'  => $tool_action,
						'alg_wc_msrp_wpnonce' => wp_create_nonce( $tool_action ),
					),
					remove_query_arg( 'alg_wc_msrp_action_done' )
				) . '"' .
				' onclick="return confirm(\'' . __( 'Are you sure?', 'msrp-for-woocommerce' ) . '\')"' .
			'>' . $tool_title . '</a>';
		}
		...
		$tools[] = '<a' .
			...
			' href="' . admin_url( 'admin.php?page=bulk-msrp-price-converter-tool' ) . '"' .
		'>' . __( 'Bulk MSRP price converter tool', 'msrp-for-woocommerce' ) . '</a>';
	...
	function get_settings() {
		...
		$admin_settings = array(
			...
			array(
				...
				'desc'     => sprintf(
					/* Translators: %s: MSRP. */
					__( 'If empty, defaults to %s.', 'msrp-for-woocommerce' ),
					'<code>' . __( 'MSRP', 'msrp-for-woocommerce' ) . '</code>'
				),
				'type'     => 'text',
		...
		$advanced_settings = array(
			...
			array(
				...
				'default'           => __( 'From %from%', 'msrp-for-woocommerce' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				'type'              => 'textarea',
				'css'               => 'width:100%;',
			...
			array(
				...
				'desc_tip'          => (
					__( 'Only show MSRP to selected user roles.', 'msrp-for-woocommerce' ) . ' ' .
				...
				'desc'              => apply_filters(
					...
					'<p>' .
						sprintf(
							'You will need %s plugin to use "User roles" option.',
